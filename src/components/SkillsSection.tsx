                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {skills.map((skillSet, idx) => (
                        <motion.div
                            ...
                            viewport={{ once: true }}
                            className="border border-gray-800 p-6 hover:border-orange-500 transition-colors duration-300"
                        >
                            <h3 className="text-orange-500 font-mono text-sm mb-6 tracking-widest">
                                {skillSet.category}
                            </h3>
                            <ul className="space-y-3">
                                {skillSet.items.map((skill, sIdx) => (
                                    <li key={sIdx} className="text-gray-400 flex items-center">
                                        <span className="w-1 h-1 bg-orange-500 mr-3" />
                                        {skill}
                                    </li>
                                ))}
                            </ul>
